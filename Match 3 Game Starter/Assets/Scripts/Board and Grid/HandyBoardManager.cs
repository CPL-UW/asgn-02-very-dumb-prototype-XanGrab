...
public class HandyBoardManager : MonoBehaviour {
	public static HandyBoardManager instance;
	...
	void Start () {
		instance = GetComponent<HandyBoardManager>();
	...
	private void CreateBoard (float xOffset, float yOffset) {
		clocks = new GameObject[xSize, ySize];
	...
	public IEnumerator FindNullTiles() {
		for (int x = 0; x < xSize; x++) {
			for (int y = 0; y < ySize; y++) {
				...
				if (!clocks[x, y].GetComponent<SpriteRenderer>().enabled) {
		...
		for (int x = 0; x < xSize; x++) {
			for (int y = 0; y < ySize; y++) {
				clocks[x, y].GetComponent<Tile>().ClearAllMatches();
			}
		}
	}
	...
	private IEnumerator ShiftTilesDown(int x, int yStart, float shiftDelay = 0.25f) {
		...
		for (int y = yStart; y < ySize; y++) {
			SpriteRenderer renderer
				= clocks[x, y].GetComponent<SpriteRenderer>();
			if (!renderer.enabled) {
				nullTiles.Add(clocks[x,y]);
			}
			shiftTiles.Add(clocks[x,y]);
		}
	...
	private GameObject GetNewTile(int x, int y) {
		...
		if (x > 0) {
			possibleColors.Remove(clocks[x - 1, y].GetComponent<Tile>().color);
		}
		if (x < xSize - 1) {
			possibleColors.Remove(clocks[x + 1, y].GetComponent<Tile>().color);
		}
		if (y > 0) {
			possibleColors.Remove(clocks[x, y - 1].GetComponent<Tile>().color);
		}
